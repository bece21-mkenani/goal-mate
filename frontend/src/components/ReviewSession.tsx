...
// Fixed card variants with proper TypeScript typing
const cardVariants = {
  ...
  useEffect(() => {
    const fetchDeck = async () => {
      try {
        ...
        const shuffledDeck = response.data.reviewDeck.sort(() => Math.random() - 0.5);
    ...
    return (
      ...
      >
        ...
        <div className="my-6 text-sm sm:text-base text-gray-600 dark:text-gray-400 text-center space-y-1">
          <p>Total Cards Reviewed: <span className="font-bold">{totalCards}</span></p>
          <p>Easy: <span className="font-bold text-green-500">{easyCount}</span></p>
          <p>Good: <span className="font-bold text-blue-500">{goodCount}</span></p>
          <p>Forgot: <span className="font-bold text-red-500">{forgotCount}</span></p>
        </div>
